...
export default function ProductPage() {
    ...
    supabase
      ...
      .then(({ data: prod, error }) => {
        ...
        if (prod) {
          ...
          const prodSizes: ProductSize[] = (prod as Product).sizes ?? [];
          ...
          if (prod.category_id) {
            supabase
              ...
              .neq('id', prod.id)
              .limit(4)
              .then(({ data: rel, error: relErr }) => {
                if (relErr) console.error('Related products error:', relErr);
  ...
  const images = product ? product.image_urls.map(resolveImageUrl) : [];
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-4">
        <div className="flex items-center gap-2 text-xs text-white/40">
          ...
          {(product as any).categories?.name && (
            <>
              <span>/</span>
              <Link to={`/shop?category=${(product as any).categories.slug}`} className="hover:text-gold-400 transition-colors">
                {(product as any).categories.name}
              </Link>
            </>
          )}
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-20">
          ...
          <div className="space-y-4">
            <div className="relative aspect-[3/4] bg-neutral-800/50 rounded-2xl overflow-hidden group border border-white/5 flex items-center justify-center">
          ...
          <div className="pt-0 lg:pt-4">
            <div className="flex items-center gap-2 mb-3">
              <p className="text-[10px] font-medium tracking-[0.2em] uppercase text-gold-400">
                {(product as any).categories?.name}
              </p>
            </div>
