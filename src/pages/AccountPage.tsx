...
function EditOrderModal({
  ...
  const validateField = (field: keyof typeof shipping, value: string) => {
    ...
    if (field === 'postal_code' && value.trim() && !/^\d{8}$/.test(value.trim())) return 'Must be exactly 8 digits';
    return '';
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <input
                  ...
                  placeholder="Postal Code (8 digits, optional)"
                ...
                {!fieldErrors.postal_code && shipping.postal_code && (
                  <p className="mt-1.5 text-xs text-neutral-500">{shipping.postal_code.length}/8 digits</p>
                )}
              </div>
...
function AddressesTab() {
  ...
  const validatePostalCode = (value: string) => {
    if (value.trim() && !/^\d{8}$/.test(value.trim())) return 'Must be exactly 8 digits';
    return '';
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    <input
                      ...
                      onChange={e => handleFieldChange(addr.id, 'postal_code', e.target.value.replace(/\D/g, '').slice(0, 8))}
                      maxLength={8}
                      inputMode="numeric"
                    />
                    {errors.postal_code && (
                    ...
                    {!errors.postal_code && form.postal_code && (
                      <p className="mt-1 text-xs text-neutral-500">{form.postal_code.length}/8 digits</p>
                    )}
                  </div>
